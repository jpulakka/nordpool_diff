...
Nordpool Diff enables you to make smarter use of quarterly electricity prices by considering how the price will change over the coming quarters. For example; the quarters before a price increase it might be smart to run the water heater or turn up the thermostat, and during the expensive quarters turn the thermostat down, thereby shifting your electricity consumption from expensive quarters to cheaper quarters.

As the share of renewables in our electricity grids increases, the price variations during each 96 quarters will be greater and greater, and also negative prices. This means that not only will you save some money by using electricity smarter, but you will also help the electricity grid. High energy prices most often mean a higher degree of dirty energy sources, so by using electricity smarter, you also reduce your carbon footprint.

What Nordpool Diff does is add a sensor to Home Assistant that will tell you how the current price relates to the prices in the coming quarters. Based on that, you can make automations in Home Assistant that take the upcoming electricity prices into account.
...
Nordpool Diff is not suitable for controlling washing machines, dishwashers and other things that needs to run for a number of quarters in a stretch to work. Neither is it suitable for things that require to be on a minimum number of quarters per day, as you won't know how for sure many quarters per day a Nordpool Diff sensor will stay over some specific threshold. If you are looking for smart charging of your EV, use something like [EV Smart Charging](https://github.com/jonasbkarlsson/ev_smart_charging).

# Installation

1. Integrate electricity prices into Home Assistant, if you haven't already. You have two choices:
 	1. [Nordpool custom component](https://github.com/custom-components/nordpool) (While the [official Nord Pool integration](https://www.home-assistant.io/integrations/nordpool) has the same name, it is otherwise completely different, and **does not work with nordpool_diff**)
...
5. You should now find a new  `sensor.nordpool_diff_triangle_40_normalize_max_min_sqrt_max` sensor that varies with the electricity price.
...
|`unit`| | `EUR/kWh/h`|  The unit of the sensor. Loosely speaking reflects rate of change (1/quarter) of quarterly price (EUR/kWh)|
...
The basics is that the price of the current quarter is compared to the upcoming quarters. The number of quarters is defined by `filter_length`. The output from the Nordpool diff sensor is a scoring in relation to the upcoming quarters. The scoring can be done in different ways, and this is where `filter_type` comes into play. The most basic variant is that each of the upcoming quarters is given equal weight. For example;
...
| quarter      | Price | Multiplier |  Sum per quarter |
|:-------------|:-----:|:----------:|:----------------:|
| 11:00-11:15  | 0.1   | -1        	| -0.1             |
| 11:15-11:30  | 0.2   | 1/4       	| 0.05     	       |
| 11:30-11:45  | 0.1   | 1/4       	| 0.025    	       |
| 11:45-12:00  | 0.4   | 1/4       	| 0.1      	       |
| 12:00-12:15  | 0.4   | 1/4       	| 0.1      	       |
| Sum (value of sensor) | | | **0.175** |

So the scoring in this case is 0.175 EUR/kWh/quarter.

This number can be thought of as an indication of how much the price is changing per quarter during the number of quarters (given by the filter length). Mathematically, this is called a FIR-filter. This FIR-filter always has -1 as the first value, and then followed by the multipliers that depend on the filter length, as follows:
...
As you see, the (rectangle) filter will weight all quarters equally. It will basically tell: how does the current electricity price compare to the average of the quarters within the filter length?

##  `filter_type: triangle`
In addition to `rectangle`, Nordpool Diff also supports `triangle` as filter type. While the `rectangle` filter puts equal weight on all future electricity price, `triangle` put greater weight on the quarters closeser in time. This can be illustrated as:
...
* `normalize: max_min_sqrt_max`: **Recommended**. Output of the filter is multiplied by square root of maximum price of the next `filter_length` quarters and divided by maximum minus minimum price of the next `filter_length` quarters. Think about it this way:
...
* `normalize: max`: Output of the filter is divided by maximum price of the next `filter_length` quarters.
* `normalize: max_min`: Output of the filter is divided by maximum minus minimum price of the next `filter_length` quarters.
* `normalize: sqrt_max`: Output of the filter is divided by square root of maximum price of the next `filter_length` quarters. This provides "somewhat scale-free normalization" where the output magnitude depends on price magnitude, but not linearly so.
...
In addition to this, you should use a filter length of 40 or more for `normalize` to work well.
...
If you have an appliance such as a heat pump, for which you would like to avoid short "on" cycles, you might get such a short cycle if you're at the end of an quarter (for example 11:59) and your automation will turn the appliance off when the next quarter starts due to the new value of the Nordpool Diff sensor, causing the appliance to only run for 1 minute.
If you have such an automation, you can benefit from the sensor attribute `next_quarter` that indicates what the value of the sensor will be next quarter.
...
2. If less than N upcoming quarters are available, then look up prices from Nordpool too.
3. Use whichever (Entso-E or Nordpool) provided more upcoming quarters.
...
### Changes to sensor value in the middle of the quarter
As you know, a new set of prices for the next day is published about 10-11 quarters before they start to come into effect. If you have a filter length of 44 or larger, you might notice that the filter for the current quarter changes in the middle of an quarter during the afternoon.
![Example](middle_change.png)

This is since the filter suddenly has new quarters to take into account, meaning it immediately makes use of new information and hence technically not an error. It might however become a problem if you are looking to avoid short on-cycles or similar.
